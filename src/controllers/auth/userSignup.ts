...
const userSignup : RequestHandler = async (req, res) => {
    ...
    try{
        ...
        const userExists= await db.query('SELECT * FROM "user" WHERE username=$1 OR email=$2',[username,email])

        if(userExists.rowCount==0){
            const user_id=generateID(10)
            const hashedPW=hashPassword(password)

            const createUser=db.query('INSERT INTO "user" ("username","user_id","hashed_pw","email") VALUES ($1,$2,$3,$4)',[username,user_id,hashedPW,email])
